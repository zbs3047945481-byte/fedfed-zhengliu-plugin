...
PREFIX="fedfed_align_xce_diag_$(date -u +%Y%m%dT%H%M%SZ)"

TMP_DIR="$(make_temp_dir)"
trap 'cleanup_temp_dir "${TMP_DIR}"' EXIT

QUEUE_FILE="${TMP_DIR}/fedfed_align_xce_diag_${PREFIX}.ps1"
LAUNCHER_FILE="${TMP_DIR}/fedfed_align_xce_diag_launcher_${PREFIX}.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/fedfed_align_xce_diag_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/fedfed_align_xce_diag_launcher_${PREFIX}.ps1"
...
function Run-Diagnostic(\$runSuffix, \$lambdaXce) {
  ...
  \$args = @(
    ...
    '--fedfed_lambda_align', '0.2',
    '--fedfed_beta_kl', '0.005',
    '--fedfed_lambda_x_ce', ([string]\$lambdaXce),
  ...
  if (\$process.ExitCode -ne 0) {
    [ordered]@{run_id=\$runId;status='failed';exit_code=\$process.ExitCode;updated_utc=(Get-Date).ToUniversalTime().ToString('o');stdout=\$stdoutPath;stderr=\$stderrPath} | ConvertTo-Json -Depth 5 | Set-Content -Encoding UTF8 \$statusPath
    throw "Diagnostic \$runId failed with exit code \$(\$process.ExitCode)"
  }
  [ordered]@{run_id=\$runId;status='succeeded';finished_utc=(Get-Date).ToUniversalTime().ToString('o');stdout=\$stdoutPath;stderr=\$stderrPath;result_dir=Join-Path \$projectPath ('result\\cifar10\\fedfed_xs_diagnostic_' + \$runId)} | ConvertTo-Json -Depth 5 | Set-Content -Encoding UTF8 \$statusPath
}

try {
  Write-QueueStatus 'running' 'FedFed align/xce diagnostic queue started.' ''
  Run-Diagnostic 'fd1p5_align0p2_xce0p4' 0.4
  Run-Diagnostic 'fd1p5_align0p2_xce0p8' 0.8
  Write-QueueStatus 'succeeded' 'FedFed align/xce diagnostic queue finished.' ''
} catch {
...
echo "FedFed align/xce diagnostic queue submitted: ${PREFIX}"
