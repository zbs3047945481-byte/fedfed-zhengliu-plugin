    ...
    for x, y in loader:
        ...
        if mode == "x":
            ...
            if mode == "xs":
                ...
                z = add_feature_noise(xs, options)
            elif mode == "xr":
                z = xr
            else:
...
def main():
    ...
    client_indices = get_each_client_data_index(dataset.train_label, options["num_of_clients"], options)
    ...
    results = [
        ...
        train_classifier(options, ConcatDataset([train_sets["x"], train_sets["xp"]]), test_sets["x"], device, "x_plus_xp_to_x"),
    ]

    out_dir = os.path.join("result", options["dataset_name"], "fedfed_six_classifier_diagnostic_" + options.get("experiment_tag", "run"))
    os.makedirs(out_dir, exist_ok=True)
    payload = {"options": options, "results": results}
    with open(os.path.join(out_dir, "diagnostic_metrics.json"), "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2)
    with open(os.path.join(out_dir, "diagnostic_summary.csv"), "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["mode", "best_acc", "final_acc", "best_epoch", "final_loss", "duration_min"])
        writer.writeheader()
        for row in results:
