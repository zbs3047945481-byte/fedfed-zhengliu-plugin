...
PREFIX="fedfed_transfer_diag_$(date -u +%Y%m%dT%H%M%SZ)"
WAIT_RUN_ID="${WAIT_RUN_ID:-fedfed_recon_noisy_diag_20260505T064210Z_recon1p0_noisy_ce_retry1}"

TMP_DIR="$(make_temp_dir)"
trap 'cleanup_temp_dir "${TMP_DIR}"' EXIT

QUEUE_FILE="${TMP_DIR}/fedfed_transfer_diag_${PREFIX}.ps1"
LAUNCHER_FILE="${TMP_DIR}/fedfed_transfer_diag_launcher_${PREFIX}.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/fedfed_transfer_diag_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/fedfed_transfer_diag_launcher_${PREFIX}.ps1"
...
function Wait-PreviousRun {
  if ([string]::IsNullOrWhiteSpace(\$waitRunId)) {
    return
  }
...
try {
  Write-QueueStatus 'waiting' 'FedFed transfer diagnostic queue submitted.' ''
  Wait-PreviousRun
  Write-QueueStatus 'running' 'FedFed transfer diagnostic queue started.' ''
  Run-Diagnostic 'B_recon0p5_no_noise' 0.5 'none' 0.0 0.0 \$false
  Run-Diagnostic 'C_recon0p2_no_noise' 0.2 'none' 0.0 0.0 \$false
  Run-Diagnostic 'D_recon1p0_noisy_ce' 1.0 'gaussian' 0.2 0.25 \$true
  Write-QueueStatus 'succeeded' 'FedFed transfer diagnostic queue finished.' ''
} catch {
...
echo "FedFed transfer diagnostic queue submitted: ${PREFIX}"
